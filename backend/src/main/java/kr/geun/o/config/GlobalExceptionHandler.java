...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ResData> baseException(BaseException e) {
        if (e.getStatus() == null) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ResData.of(e.getMessage()));
        }

        return ResponseEntity.status(e.getStatus()).body(ResData.of(e.getMessage()));
    }
}
